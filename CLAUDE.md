...
- **v0.5.132** — Four fixes surfaced by compiling TypeScript v6.0.3 (`src/tsc/tsc.ts`): (1) `hir.functions` duplicate-entry deduplication in `compile_module` — a namespace's non-exported inner function sharing a func_id with a module-level function caused LLVM "invalid redefinition of function" for `createSourceFile` in `parser.ts`; added `compiled_funcs` HashSet guard before `compile_function` and `emitted_func_wrappers` guard before the `__perry_static_closure_`/`__perry_wrap_` pair in the wrapper loop. (2) `collect_closures_in_expr` missing `Expr::ObjectDefineProperty` — the descriptor arg (a `{ value: fn, get: fn, ... }` object literal) can embed closures; the pre-walk's `_ => {}` catch-all silently skipped it, leaving `@perry_closure_...__71` undefined at link time in `scanner.ts`. Added walk arms for `ObjectDefineProperty`, `ObjectGetOwnPropertyDescriptor`, the single-operand `Object.*` helpers, and `ReflectDefineProperty`. (3) `.match(importedRegex)` 0-arg error in `watch.ts` — `args.remove(0)` extracts the regex into `regex_expr`, but the follow-up `matches!(&regex_expr, Expr::RegExp{..}|Expr::LocalGet(_))` check excluded `Expr::GlobalGet` (what imported module-level regexes become), leaving `args` empty before falling through; removed the overly-restrictive type guard and always emit `Expr::StringMatch`. (4) Added `vendor/TypeScript` as a git submodule at tag `v6.0.3` for reproducible build testing.
