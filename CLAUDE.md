...
- **v0.5.140** — Fix nested function calls returning 0.0: JavaScript function hoisting was not being implemented properly — calls to nested functions before their declaration were resolving to the uninitialized local (0.0). Added `pre_register_and_hoist_nested_functions()` in `lower_decl.rs` which pre-registers all nested function declarations and hoists their `Let` statements to the beginning of the block, ensuring they are initialized before any code that calls them (matching JavaScript's function hoisting semantics).
