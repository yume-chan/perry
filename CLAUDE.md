...
- **v0.5.131** — Fix `Expr::FuncRef` to emit a static `ClosureHeader` global (`__perry_static_closure_<name>`) per user function instead of calling `js_closure_alloc` on every read. Each read of the same named function now returns the same pointer, so `fn === fn` works correctly. Changed in `codegen.rs` (add static constant after the `__perry_wrap_` loop for `hir.functions`, and also inside `compile_static_method` for static class methods) and `expr.rs` (`FuncRef` now loads the global address and NaN-boxes it, matching the `ExternFuncRef` pattern).
